...
// @ts-ignore
const provider_1 = __importDefault(
  require("web3-provider-engine/subproviders/provider")
);
// @ts-ignore
const rpc_1 = __importDefault(require("web3-provider-engine/subproviders/rpc"));
// @ts-ignore
const websocket_1 = __importDefault(
  require("web3-provider-engine/subproviders/websocket")
);
...
class HDWalletProvider {
  constructor(...args) {
    ...
    const _a = (0, getOptions_1.getOptions)(...args),
      {
        provider,
        url,
        providerOrUrl,
    ...
    if (
      __classPrivateFieldGet(this, _HDWalletProvider_addresses, "f").length ===
      0
    ) {
      throw new Error(
    ...
    const tmpAccounts = __classPrivateFieldGet(
      this,
      _HDWalletProvider_addresses,
      "f"
    );
    const tmpWallets = __classPrivateFieldGet(
      this,
      _HDWalletProvider_wallets,
      "f"
    );
  ...
  checkBIP39Mnemonic({ addressIndex, numberOfAddresses, phrase, password }) {
    ...
    const hdwallet = (0, hdwallet_1.createAccountGeneratorFromSeedAndPath)(
      (0, bip39_1.mnemonicToSeedSync)(phrase, password),
    ...
    for (let i = addressIndex; i < addressIndex + numberOfAddresses; i++) {
      ...
      const addr = `0x${Buffer.from(
        (0, hdwallet_1.uncompressedPublicKeyToAddress)(wallet.publicKey)
      ).toString("hex")}`;
      __classPrivateFieldGet(this, _HDWalletProvider_addresses, "f").push(addr);
      __classPrivateFieldGet(this, _HDWalletProvider_wallets, "f")[addr] =
        wallet.privateKey;
    }
  }
  ...
  ethUtilValidation({ addressIndex, privateKeys }) {
    ...
    for (let i = addressIndex; i < privateKeys.length; i++) {
      ...
      if (EthUtil.isValidPrivate(privateKey)) {
        ...
        __classPrivateFieldGet(this, _HDWalletProvider_addresses, "f").push(
          address
        );
        __classPrivateFieldGet(this, _HDWalletProvider_wallets, "f")[address] =
          privateKey;
      }
    }
  }
...
(_HDWalletProvider_wallets = new WeakMap()),
  (_HDWalletProvider_addresses = new WeakMap());
